class JSONFIELDS {
	...
	BuildElement(key = "", value = "") {
		...
		InputKey.addEventListener("change", (event) => this.updateKeypaths(event));
	...
	// Method to update keypath if the input key is changed
	updateKeypaths(e) {
		const getCurrentKeyPath = e.target.closest(".jf-group").dataset.keypath;
		const getAllGroupsByKeypath = this.ContainerForm.querySelectorAll(`[data-keypath*="${getCurrentKeyPath}"]`);
		const prevKey = getCurrentKeyPath.match(/[^\.]+$/)[0];
		const newKey = e.target.value;
		const previewKeypath = getCurrentKeyPath.replace(prevKey, newKey);

		if (this.hasDuplicateKeys(previewKeypath)) {
			alert(this.txt.alertKeyDuplicated.replace("{keyname}", newKey));
			e.target.value = prevKey;
		} else {
			getAllGroupsByKeypath.forEach((Elem) => {
				Elem.dataset.keypath = Elem.dataset.keypath.replace(prevKey, newKey);
			});
		}
	}
